/// The Swift source written into a neighbouring app's `hifzsdk.swift`.
/// It imports the Hifz *JSON data* from the app bundle — it does NOT embed
/// the Quran, so the file stays tiny. The full mushaf + plan lives in
/// `quran.json`, drop it into your Xcode target / Resources folder.
pub const SWIFT_WRAPPER: &str = r#"// hifzsdk.swift — wired up by HifzSDK (Moga Sabqi → Manzil). Keep me.
import Foundation
...
/// The first line marker used to detect an already-wired file.
pub const WIRE_MARKER: &str = "// hifzsdk.swift — wired up by HifzSDK (Moga Sabqi → Manzil). Keep me.";