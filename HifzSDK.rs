// HifzSDK.rs — the whole HifzSDK in ONE Rust file (Sabqi → Manzil).
// Rust-only. Same SDK as HifzSDK.swift. Needs only serde_json.
// 
// Regenerate data/quran.json with:  cargo run -- --build-data
//


use serde_json::{json, Map, Value};

pub const BASE_JSON: &str = "data/quran.base.json";
...
fn main() {
    if std::env::args().any(|a| a == "--build-data") {
        let combined = full_export();
        let pretty = serde_json::to_string_pretty(&combined).expect("plan serializes");
        std::fs::write("data/quran.json", pretty).expect("write data/quran.json");
        println!("[HifzSDK.rs] wrote data/quran.json (mushaf + sabqi/manzil plan).");
        return;
    }
    println!("[HifzSDK.rs] Sabqi → Manzil SDK loaded. Run with --build-data to regenerate quran.json.");
}
